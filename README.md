...
The `rust-consumer` test is the critical one - it uses the same dependencies (serde_json, rand) as the library and is built with two toolchains: a different one from the library (beta) and the same one (stable). With a *different* toolchain on Linux (rust-lld) and Windows (link.exe: LNK2005 + LNK1169), linking an unpatched library fails with duplicate symbol errors, so that is the case that discriminates patched from unpatched. Two cases do not fail this way, and so cannot prove patching works on their own: with the *same* toolchain the linker resolves the identical archive members first-definition-wins, and on macOS ld64 does so regardless of toolchain, so an unpatched library still links. Symbol hiding is still worth doing on macOS for symbol-table hygiene (a clean final binary or dylib, no re-export clashes), it just is not link-breaking. With patching, every non-API symbol is hidden, so the consumer links regardless of its toolchain.
