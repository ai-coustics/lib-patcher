//! Backward-compatibility blocklist code path.
//!
//! Before lib-patcher switched to the allowlist model (`--keep-prefix`: keep
...
//! reached when a caller supplies the old blocklist flags.
...
use crate::{detect_archive_arch, list_symbols, target_os_from_triplet};
...
/// Patches a static library to hide specific symbols (deprecated blocklist mode).
///
/// This is the pre-allowlist behavior: every symbol in `symbols_to_hide` is
/// localized, and everything else is left public. Prefer
/// [`crate::patch_lib`] (allowlist mode) for new code.
///
/// # Arguments
///
...
///
/// ```rust,no_run
/// use lib_patcher::{patch_lib_blocklist, default_symbol_blocklist};
/// use std::path::Path;
///
/// patch_lib_blocklist(
...
/// Verifies that a patched library meets the expected criteria (blocklist mode).
///
/// # Arguments
///
